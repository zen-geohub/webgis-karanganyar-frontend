...
(function (factory, window) {
    ...
    function donut(options) {
        var div = doc.createElement('div'),
            ...
            borderWidth = options.borderWidth || 1, // New option for border width
            borderStyle = options.borderStyle || 'solid', // New option for border style
            borderColor = options.borderColor || '#000000', // New option for border color
            starFillPercentage = options.starFillPercentage || 0,
            r = size / 2,
            PI = M.PI,
            sin = M.sin,
            cos = M.cos,
        ...
        var outerCircle = doc.createElementNS(NS, 'circle');
        outerCircle.setAttribute('cx', size0 / 2.0);
        outerCircle.setAttribute('cy', size0 / 2.0);
        outerCircle.setAttribute('r', r + 3); // Adjust radius as needed
        outerCircle.setAttribute('stroke', borderColor);
        outerCircle.setAttribute('stroke-width', borderWidth);
        outerCircle.setAttribute('fill', 'none');
        outerCircle.setAttribute('stroke-dasharray', borderStyle === 'dashed' ? '4, 2' : 'none');
    ...
    function createDonut(points, opt, cfgFn) {
        ...
        var myDonut = donut({
            size: cfg.size || 50,
            weight: cfg.weight || 0,
            fillOpacity: cfg.fillOpacity || 0.7,
            strokeOpacity: cfg.strokeOpacity || 0.7,
            data: list,
            onclick: cfg.onclick,
            colors: cfg.colors,
            ...
            starFillPercentage: cfg.starFillPercentage
        });
        myDonut.config = cfg;
        return myDonut;
    }
        ...
        var createIcon = function (cluster) {
            ...
            var myDonut = createDonut(markers, donutOpt, function (points) {
                ...
                return {
                    ...
                    starFillPercentage: style.starFillPercentage
                }
            })
